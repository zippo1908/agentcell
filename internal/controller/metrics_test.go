...
func TestCellMetricsReflectActiveSessionsAndTotal(t *testing.T) {
	...
	// A real terminal phase. "Succeeded" is not one this API has: the test
	// passed only because observeSessions counts an allow-list, so an unknown
	// phase happens to fall outside it. That is a coincidence, not a check.
