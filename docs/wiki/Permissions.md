...
**已连接的账号借不了**,而且这不是保守,是它做不到:OAuth 的刷新令牌是**轮换**的
——用一次就换一个新的、把旧的作废。两个运行时拿着同一份账号,谁先刷新谁就把对方
踢下线。这正是 `internal/controller/account_sync.go` 存在的理由,借出等于故意把那个
bug 请回来。

要共享就借一把 **API key**:它是静态的,每次都是同一个字符串,这才是它能被借的原因。
或者让对方自己连一次账号,三十秒。
