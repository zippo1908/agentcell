...
项目(Cell)
  ...
  └── 会话配额(maxSessions:这里同时能有几条会话占槽位)
       └── 会话 ── owner:一个真人,创建时写一次、终身不变
            │         他的凭据为这条会话的每一轮买单
            ├── operator:项目里能派工的人,针对**共享**会话(黑板那条)
            │             ——他们打字,但不付钱
...
出资人,凭据也不会在出借者不知情时被换掉。

按用户分配的 Unix uid、0700 目录、一人一个 tmux server,是**默认的隔离
拓扑**,不是产品原则:它们让会话默认互不干扰,但不构成「会话不能被有意
共享」的理由。
...
| **每人每项目一条活会话**(路由规则),追问按顺序排队、唤醒后逐条送达;`maxSessions` 数的是**会话**——一场共享对话是一个槽位,不管里面有两个人还是十个人 | ✅ 已验证 |
